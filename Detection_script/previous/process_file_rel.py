...
def screen_image_absolute(metrics):
    ...
    MOTION_BLUR_MAX = 0.08  # Motion blur > 0.08 = significant motion
    ...
    EDGE_DENSITY_MIN = 0.05  # Too smooth
    ...
    # Decision logic
    if any(f in flags for f in ["blur", "motion_blur", "low_texture_ratio", "saturation_ratio"]):
        decision = "REPAIR"
...
def test_single_image(image_path, target_size=None, resize_mode="pad"):
    ...
    try:
        ...
        # Screen using ABSOLUTE thresholds (no dataset needed)
        decision = screen_image_absolute(quality)

        print("\n" + "-" * 60)
        print("SCREENING ANALYSIS (Absolute Thresholds):")
        print("-" * 60)

        # Show which thresholds were exceeded
        print("\nThreshold checks:")
        print(f"  {'✓' if quality['blur'] >= 100 else '✗'} Blur variance:     {quality['blur']:.1f} (threshold: ≥100)")
        print(
            f"  {'✓' if quality['motion_blur'] <= 0.08 else '✗'} Motion blur:       {quality['motion_blur']:.4f} (threshold: ≤0.08)")
        print(
            f"  {'✓' if 0.15 <= quality['brightness'] <= 0.85 else '✗'} Brightness:        {quality['brightness']:.3f} (threshold: 0.15-0.85)")
        print(
            f"  {'✓' if quality['contrast'] >= 0.1 else '✗'} Contrast:          {quality['contrast']:.3f} (threshold: ≥0.1)")
        print(
            f"  {'✓' if quality['saturation_ratio'] <= 0.1 else '✗'} Saturation ratio:  {quality['saturation_ratio']:.4f} (threshold: ≤0.1)")
        print(
            f"  {'✓' if quality['low_texture_ratio'] <= 0.15 else '✗'} Low texture ratio: {quality['low_texture_ratio']:.4f} (threshold: ≤0.15)")
        ...
        if decision["decision"] == "REPAIR":
            ...
            if "blur" in decision["flags"]:
                print(f"  • Image is BLURRY (variance: {quality['blur']:.1f} < 100)")
                print("    → Inpainting will regenerate sharp details")
            if "motion_blur" in decision["flags"]:
                print(f"  • Image has MOTION BLUR (score: {quality['motion_blur']:.4f} > 0.08)")
                print("    → Inpainting will remove blur artifacts")
            if "low_texture_ratio" in decision["flags"]:
                print(f"  • Image has LOW TEXTURE areas ({quality['low_texture_ratio'] * 100:.1f}% > 15%)")
                print("    → Inpainting will add detail to flat regions")
            if "saturation_ratio" in decision["flags"]:
                print(f"  • Image has SATURATED pixels ({quality['saturation_ratio'] * 100:.1f}% > 10%)")
                print("    → Inpainting will fix clipped regions")
            if "brightness_extreme" in decision["flags"]:
                print(f"  • Image has EXTREME BRIGHTNESS ({quality['brightness']:.3f})")
                print("    → May need exposure correction")
            if "low_contrast" in decision["flags"]:
                print(f"  • Image has LOW CONTRAST ({quality['contrast']:.3f} < 0.1)")
                print("    → Image appears flat/washed out")
            ...
            print("\nReasons:")
            print(f"  • High entropy ({quality['entropy']:.2f} > 6.0)")
            print(f"  • High edge density ({quality['edge_density']:.4f} > 0.05)")
            ...
            print("\nReasons:")
            print("  • Image quality meets all thresholds")
